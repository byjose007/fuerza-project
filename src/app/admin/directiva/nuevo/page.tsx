...
export default function NuevoMiembroPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from("directiva").insert([
        {
      ...
      setError(
        err.message ||
          "Error al guardar el miembro. Por favor, intenta de nuevo.",
      );
    } finally {
              ...
              {preview ? (
                <div className="space-y-1 text-center w-full flex flex-col items-center justify-center">
                  ...
                  <div className="relative z-10 flex flex-col justify-end h-full w-full pb-4">
                    <label
                      htmlFor="file-upload"
                      className="cursor-pointer bg-white/90 backdrop-blur-md rounded-lg font-bold text-[var(--color-brand-magenta)] hover:bg-white px-4 py-2 shadow-sm border border-gray-200 text-sm mx-auto"
                    >
                      Cambiar foto
                      <input
                        id="file-upload"
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Números menores se muestran primero (Ej: 0 para Presidente, 1
                Vicepresidente, etc).
              </p>
            </div>
          </div>
