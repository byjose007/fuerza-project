...
export function AjustesForm({ initialSettings }: Props) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updates = [
        {
          key: "home_message_enabled",
          value: settings.home_message_enabled.toString(),
        },
  ...
  return (
    <form onSubmit={handleSave} className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">
                Mensaje de Presidencia (Home)
              </h2>
              <p className="text-sm text-gray-500">
                Bloque de citación destacado en la página de inicio.
              </p>
            </div>
          </div>
          ...
          <label className="flex items-center cursor-pointer">
            <div className="relative">
              <input
                ...
                onChange={(e) =>
                  handleChange("home_message_enabled", e.target.checked)
                }
              />
              <div
                className={`block w-14 h-8 rounded-full transition-colors ${settings.home_message_enabled ? "bg-[var(--color-brand-magenta)]" : "bg-gray-300"}`}
              ></div>
              <div
                className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${settings.home_message_enabled ? "transform translate-x-6" : ""}`}
              ></div>
            </div>
        ...
        <div
          className={`p-6 space-y-6 ${!settings.home_message_enabled ? "opacity-50 pointer-events-none" : ""}`}
        >
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                handleChange("home_message_text", e.target.value)
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleChange("home_message_author", e.target.value)
              }
